...
stim_duration_rel = 0.3 ; % In percentage
window_rms = 0.02;% In seconds
rms_threshold_gain_1 = 1.2;
rms_threshold_gain_2 = 1.2;

STATE = 0;

STIMULATION_OUTPUT = [];
STIMULATION_OUTPUT = zeros(trial_length,2);
emg_rms_long = zeros(trial_length,2);
EMG_burst_detected = [];
EMG_burst_predicted = [];
emg_seg = [];
...
while(w < trial_length)
    ...
    switch(STATE)
        ...
        case 1
            ...
            emg_rms_long(w:w + window_recording,1) = rms(EMG_raw(w:w + window_recording,1));
            emg_rms_long(w:w + window_recording,2) = rms(EMG_raw(w:w + window_recording,2));
        ...
        case 2
            if(k < trial_length)
                ...
                k = w;
                emg_rms_long(w:w + window_stim,1) = emg_rms_th(1);
                emg_rms_long(w:w + window_stim,2) = emg_rms_th(2);
...
emg_rms_short = [];
emg_rms_short(:,1) = rms_mov(EMG_raw(:,1), round(fs)*0.02, 0, 0)';
emg_rms_short(:,2) = rms_mov(EMG_raw(:,2), round(fs)*0.02, 0, 0)';
t_rms_short = 0:1/50:(length(EMG_fil_h_env)-1)/fs;
hold on
%%
figure,
...
plot(t,emg_rms_long(:,1), 'linewidth', 2.5)
plot(t_rms_short,emg_rms_short(:,1), 'linewidth', 1.5)
plot(t,emg_rms_long(:,2), 'linewidth', 2.5)
plot(t_rms_short,emg_rms_short(:,2), 'linewidth', 1.5)
